...
# 保存数据集
save_dataset(dataset, './03_data/processed/train_data.npz')

# 创建跨负载场景的 DataLoader
config = {
    'data_path': './03_data/processed',
    'batch_size': 32,
    'num_workers': 4,
    'scenario_config': {
        'train_loads': [0, 1],
...
# 生成报告
report = checker.generate_report('./01_notes/leakage_report.txt')
...
has_leakage = quick_leakage_check(
    ...
    report_path='./01_notes/quick_check.txt'
)
```
...
```bash
cd E:\硕士学习记录\研零\论文刷题\paper
python 02_code/test_data_module.py
```
