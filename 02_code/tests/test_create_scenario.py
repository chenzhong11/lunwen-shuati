...
# `tests/` 位于 `02_code/` 下；项目根目录是 `paper/`。
...
def test_scenario_s1_to_s6():
    ...
    测试 S1-S6 场景创建
    """
    data_path = str(PROJECT_ROOT / "03_data" / "raw")
    
    mat_files = list(Path(data_path).glob('*.mat'))
    if not mat_files:
        pytest.skip(f"03_data/raw 不存在可用 .mat 文件: {data_path}")
...
if __name__ == '__main__':
    ...
    if list(Path(data_path).glob('*.mat')):
        ...
        print(f"\n[SKIP] S1-S6真实数据场景测试：03_data/raw 不存在可用 .mat 文件")
