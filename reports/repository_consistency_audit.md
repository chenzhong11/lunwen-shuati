...
1. `reports/preflight_check.md` 将基础协议检查写成“PASS / 可执行实验”，掩盖了训练、评估和结果生成尚未打通。
2. README 的快速开始把 02–05 脚本写成完整复现实验流程，但 02、03、04、05 原先分别只是打印步骤、创建场景、批量检查场景和打印路径。
3. `tests/test_create_scenario.py` 在真实数据缺失时通过 `return None` 表示跳过，手工汇总仍可能输出“所有测试通过”；而 `data/raw/` 空目录也可能被当作可用数据目录。
...
7. `.gitignore` 中未锚定的 `data/` 规则误忽略了 `src/data/`，导致正式数据模块源文件没有被 Git 跟踪。
...
| 忽略规则 | `.gitignore` 改为只忽略仓库根目录 `/data/`，不再忽略 `src/data/` |
...
- `scripts/03_run_scenario.py --method wdcnn_adabn`：当前明确返回未实现，不会伪造结果。
...
| 位置 | 当前值 | 应属用途 | 是否冲突 | 处理 |
|---|---:|---|---|---|
| `config/default.yaml:data.sample_length` | 2048 | 正式 Phase 1 数据协议 | 否 | 保留为正式标准 |
| `config/default.yaml:model.input_length` | 2048 | 正式 WDCNN | 否 | 保留为正式标准 |
| `scripts/03_run_scenario.py`、`04_run_all_scenarios.py` | 2048 | 正式场景入口/协议检查 | 否 | 保留并显式传入 |
| `src/models/wdcnn.py` | 2048 | 正式模型默认输入 | 否 | 保留 |
| `config/data_config.yaml` | 1024 | legacy/generic 数据模块配置 | 原表述会冲突 | 已标注，不属于 Phase 1 |
| `docs/DATA_MODULE_GUIDE.md` | 1024 | legacy/teaching API 示例 | 原表述会冲突 | 已标注，不属于 Phase 1 |
| `examples/data_module_example.py` | 1024 | synthetic generic example | 原表述会冲突 | 已标注，不属于 Phase 1 |
| `data/processed/*.npz` | 1024 | 历史本地样例 | 不属于正式协议 | 保留，不上传、不删除 |

## 7. 下一步最小闭环

当前 S1 baseline 的准备条件已经形成，但本机尚未有真实 CWRU 文件。缺口只有：

1. 准备 `data/raw/` 中正式映射所需的 CWRU `.mat` 文件；
